...
- Linux: `lscpu`（システム情報取得）、任意で `nvidia-smi`（GPU情報）
- Windows: PowerShell（標準搭載）、任意で `nvidia-smi`（GPU情報）
...
4. **スコア算出** — 独自スコアをASCIIバーチャートで表示
   - `CPU Single` … シングルコア性能
   - `CPU Multi` … 全スレッド性能
   - `GPU V-Calc` … NVIDIA GPU検出時のみ。`最大SMクロック × VRAM(GB)^1.5` ベースの仮想スコア（VRAM容量を重視）。非NVIDIA環境では安全にスキップ。
...
GPU V-Calc = 最大SMクロック(MHz) × (VRAM_GB ^ 1.5) / 10
```
