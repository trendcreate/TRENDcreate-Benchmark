...
CPU_MHZ="$(get_lscpu 'CPU max MHz')"
[ -z "$CPU_MHZ" ] && CPU_MHZ="$(get_lscpu 'CPU MHz')"
CPU_MHZ="$(echo "$CPU_MHZ" | grep -oE '[0-9]+' | head -n1)"   # 整数部のみ
[ -z "$CPU_MHZ" ] && CPU_MHZ="0"
...
if [ "$CPU_MHZ" != "0" ]; then
    printf "  ${C_GREEN}%-20s${C_RESET}: ${C_BOLD}%s MHz${C_RESET}\n" "動作クロック(最大)" "$CPU_MHZ"
fi
printf "  ${C_GREEN}%-20s${C_RESET}: ${C_BOLD}%s${C_RESET}\n" "L3キャッシュ容量" "$L3_CACHE"
if [ "$HAS_GPU" = "1" ]; then
...
# ============================================================================
"$PY" - "$NPROC" "$HAS_GPU" "$GPU_VRAM_MB" "$GPU_SMCLK_MHZ" "$CPU_MODEL" "$GPU_NAME" "$SCORE_DIR" "$ARCH" "${CORES:-N/A}" "$L3_CACHE" "$GPU_VENDOR" "$CPU_MHZ" <<'PYEOF'
...
CPU_MHZ    = int(sys.argv[12]) if len(sys.argv) > 12 and sys.argv[12].isdigit() else 0
...
record = {
    ...
    "system": {
        ...
        "cpu_mhz": CPU_MHZ if CPU_MHZ > 0 else None,
        "l3_cache": L3_CACHE,
